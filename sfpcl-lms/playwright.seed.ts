...
const CRITICAL_UAT_SPEC = 'critical-uat-smoke.e2e.spec.ts';
const SPEC_ARGUMENT = /(?:\.e2e\.spec\.ts|\.spec\.ts)$/;

export type FixtureFamily = 'staff' | 'portal' | 'epic009' | 'critical';
...
export function requiredFixtureFamilies(arguments_: readonly string[]): FixtureFamily[] {
  ...
  const selectsCriticalUat = selectedSpecs.some(argument => argument.includes(CRITICAL_UAT_SPEC));
  const selectsAnotherSpec = selectedSpecs.some(argument => !argument.includes(EPIC_009_SPEC));

  if (selectsCriticalUat) {
    return ['staff', 'portal', 'epic009', 'critical'];
  }
