...
import { capitaliseInterest, canCapitaliseInterest, canGenerateAccrual, canGenerateInvoice, fetchInterestInvoices, generateInterestInvoice, previewInterestCapitalisations, runInterestAccrual, type AccrualRunProjection, type CapitalisationPreview, type InterestCapitalisationProjection, type InterestInvoiceProjection } from '../../services/servicingApi';
...
const InterestManagement: React.FC = () => {
  ...
  const refreshInvoices = async () => setInvoices((await fetchInterestInvoices()).items);

  useEffect(() => {
    let active = true; setLoading(true); setError(null);
    void Promise.all([fetchLoanAccounts(1, 100), fetchInterestInvoices()]).then(([loanPage, invoicePage]) => {
      if (!active) return; setAccounts(loanPage.items); setSelectedLoan(loanPage.items[0]?.loan_account_id ?? ''); setInvoices(invoicePage.items);
  ...
  const runAccrual = () => act(async () => setAccrual(await runInterestAccrual(month, accounts.map(row => row.loan_account_id), keyFor(`accrual:${month}`))));
