...
describe('mounted witness resource actions', () => {
  it('captures with the exact body and refetches the canonical collection once', async () => {
    ...
    expect(create).toHaveBeenCalledWith('application-1', { member_id: 'member-1', witness_name: 'Witness One', pan: 'ABCDE1234F', aadhaar: '123412341234' });
  });
  ...
  it('corrects with current version and never exposes a control when the resource action is absent', async () => {
    ...
    await waitFor(() => expect(update).toHaveBeenCalledWith('application-1', 'witness-1', { version: 1, witness_name: 'Corrected' }));
    view.unmount();
