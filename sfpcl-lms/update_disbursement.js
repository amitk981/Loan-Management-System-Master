...
const formatDisbBadge = (stage: DisbStage) => {
  switch (stage) {
    ...
    case 'cfc_pending': return 'CFC authorisation pending';
...
const DisbursementHub: React.FC<DisbursementHubProps> = ({ onOpenApplication, initialSelectedId }) => {
  ...
  useEffect(() => {
    if (app) {
      if (app.sapCustomerCode || app.id === 'l003') { // LO00000035
    ...
    <div className="p-6 space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">{disbQueue.length} applications in disbursement workflow</p>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="card p-0 overflow-hidden lg:col-span-1">
            <div className="p-4 bg-slate-50 border-b border-slate-200">
              <p className="text-xs font-semibold text-slate-500 uppercase tracking-wide">Finance disbursement queue ({disbQueue.length})</p>
            </div>
            <div className="divide-y divide-slate-100">
              {disbQueue.map(a => {
                let aStage: DisbStage = a.sapCustomerCode ? 'bank_verify' : 'sap_pending';
                if (a.id === selected) aStage = stage;
                ...
                return (
                  ...
                  >
                    ...
                    <StatusBadge label={formatDisbBadge(aStage)} size="sm" />
                  </button>
                );
              })}
            </div>
          ...
          {app && (
            <div className="lg:col-span-2 space-y-4">
                    ...
                    <div className="space-y-4">
                      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3">
                        {[
                          ...
                          { label: 'Approval authority satisfied', ok: true },
                          { label: 'Exception approvals complete', ok: true },
                          { label: 'Documentation checklist verified', ok: true },
                          { label: 'Security complete', ok: true },
                          { label: 'SAP code confirmed', ok: sapConfirmed },
                          { label: 'Bank details verified', ok: bankVerified },
                          { label: 'Disbursement amount within sanction', ok: true },
                          { label: 'No unresolved blocker', ok: true }
                        ].map(item => (
              ...
              {(stage === 'initiated' || stage === 'cfc_pending' || stage === 'completed') && (
                <div className="card">
                  <h3 className="text-sm font-semibold text-slate-700 mb-3 flex items-center gap-2">
                  ...
                  {stage !== 'initiated' ? (
                    <div className="flex items-center gap-2 text-green-700 bg-green-50 rounded-lg p-3 text-sm">
                      <Check size={16} /> Payment initiated successfully. Pending CFC authorisation.
                    </div>
                  ) : (
                    <div className="space-y-4">
                      ...
                      <div className="flex gap-3">
                        {isSeniorManager ? (
                          <>
                            <button className="btn-primary text-sm flex-1" disabled={!paymentNarration.includes(app.applicationNumber)} onClick={() => { setStage('cfc_pending'); auditLog('INITIATED_PAYMENT_SENT_TO_CFC'); }}>Initiate payment & Send to CFC</button>
                  ...
                  {stage === 'completed' ? (
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="flex flex-wrap gap-3">
                        {isCfc ? (
                          <>
                            <button className="btn-primary text-sm flex-[2]" disabled={!utrReference || !cfcBankEvidence} onClick={() => { setStage('completed'); auditLog('AUTHORISED_TRANSFER', utrReference); }}>Authorise transfer</button>
...
fs.writeFileSync('/Users/amitkallapa/Downloads/New Loan Management Designs/sfpcl-lms/src/pages/disbursement/DisbursementHub.tsx', code);
