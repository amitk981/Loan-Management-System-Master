...
| Mock notification/profile data | Staff could not see real S04 inbox state or backend identity details | `NotificationsCenter`, `MyProfile` | Closed for staff in 003IA with `GET /api/v1/notifications/`, mark-read persistence/audit, and read-only My Profile rendering from `/api/v1/auth/me/`. Borrower portal notifications remain future work. |
