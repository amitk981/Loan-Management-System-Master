...
- 002EX must consume that 002E bridge unchanged: no tracer screen before backend login and `/auth/me/`; no demo-auth bypass for smoke evidence; empty-permission sessions must not see tracer actions.
- 002EY should make Playwright prove the same production/default auth path. Its login flow must hit the dev Django server and should include a negative browser check that a missing/revoked session lands on staff login without exposing tracer routes.
