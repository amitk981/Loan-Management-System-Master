...
Normal and repair runs use `quality_gates.backend_validation_policy: selective`. The validator records its classification, binding lane, reason, changed paths, and independently derived impacted labels in `backend-validation-lane-results.md`. A localized Low/Medium backend change runs every changed test module, every repository test module whose parsed Python imports reference the changed production root, and the root's maintained route/contract ownership patterns in `scripts/config/ralph-backend-test-impact.json`. Missing or invalid ownership fails closed to complete coverage. Complete coverage is also required for High risk, every fourth completed product slice, migrations/models/configuration/routing/dependencies, package-root or shared-root changes, multiple backend modules, broad changes, deletions/renames, missing tests, or tests that cannot be mapped to the changed module. Frontend/docs-only candidates retain the cheap Django and migration-consistency checks but skip backend test/coverage execution except at the periodic full checkpoint. The full checkpoint retains the complete label and 85% floor; an impacted lane never claims global coverage.

Backend gates are ordered from cheap to expensive. A failed Django system check, migration consistency check, required browser contract, or impacted regression pack records the failure and defers complete coverage to the repair attempt. Both impacted and complete test lanes use Django fail-fast only after the candidate is red; green candidates still execute their entire selected label set. This changes failure latency, not passing acceptance scope.
