...
- `architecture-review`: one of two mode-pure independent roles. `review_discovery` runs at the mandatory periodic checkpoint or an Epic/project boundary and reviews the fixed merged diffs for test quality, source fidelity, duplication, and architecture drift; it may admit root-grouped corrective work under the severity rules below. `review_closure` runs only for inherited finding identities and exact reproducers; it cannot discover unrelated findings or expand the queue. Neither role modifies production code. Every product slice still receives its own fixed-spec review packet and independent validation before commit.

Owner preparation keeps the runnable queue concrete before unattended execution; implementation does not reopen those specification decisions. Closure review is enforced independently: its manifest must equal the authenticated inherited Finding ID/Root ID set, report every New severity as zero, and replay the exact command frozen from the completed repair slice. Fresh evidence paths are allowed; broader or narrower substitute probes are rejected. If terminal closure coincides with an Epic/project boundary, Ralph preserves that boundary and runs discovery next instead of declaring completion.

Periodic architecture discovery remains fail-closed: every four completed slices until two clean reviews expand the interval to eight, with any later Critical/High finding resetting it to four. Epic and final-project boundaries also set discovery due. Authenticated inherited closure verification remains a separate mandatory role. Boundary ownership comes from each slice's explicit `## Parent Epic` metadata, including CR slices, and Ralph does not close an Epic while any Not Started or Blocked slice owned by it remains.

An explicit owner-maintainer completion baseline is the narrow exception to periodic queue barriers. The trusted `architecture_review_completion_baseline` record in `.ralph/state.json` freezes an exact ordered set of already-planned product slices. While its status is `active`, Ralph selects only unfinished slices from that set and defers non-Critical architecture discovery, existing High corrections, finalizers, and terminal-repair work. Critical architecture state remains fail-closed. Completing the last listed slice atomically moves the baseline to `awaiting_review`; no product or corrective slice is then selectable until one validated consolidated architecture review records the baseline as `reviewed`. Review-generated corrections remain in the queue and resume afterward. See ADR-0006.

`review-packet.md` convergence metrics and its semantic finding manifest are machine-validated before a review can merge. Corrective generations are tracked independently by stable `Root ID`, never by one global Epic/review counter: a new root begins at generation one, remapping the same root to a different corrective advances only that root, reusing the same pending corrective consumes no generation, and a `Closed` row removes that root's state. Each root admits one grouped corrective and at most one successor; closed or unrelated roots cannot exhaust another root's budget. After the same root reaches that cap, only a High-risk CR with an exact `## Architecture Review Finalizer` declaration (`Epic`, `Root ID`, and exhausted generation) may close it. The owner has preauthorized exactly one terminal finalizer per stable Root ID through the protected `[approved-finalizer-policy]`; exact legacy `[approved-finalizer]` entries remain supported. An independently validated review may therefore queue one terminal CR instead of exiting at the deterministic cap. If a reviewer first emits an ordinary successor, Ralph retains that quarantined review and performs one bounded metadata-only rewrite into the terminal CR instead of repeating the critique or terminating immediately. A terminal run must replay every original review probe's exact command and retain current-run green evidence; a narrower proxy regression cannot close the root. The transition retains the exhausted generation rather than inventing generation three and records every Finding ID/Root ID pair grouped into the terminal finalizer, not only its primary root. If executable review evidence disproves that finalizer, the protected `[approved-terminal-repair-policy]` permits one active High-risk `## Architecture Review Recurrence Repair` episode for the same finalizer contract. Ralph retains unrelated validated findings, blocks ordinary product work behind the exact repair, and keeps the episode `awaiting_verification` after product gates pass. The authenticated pending repair may depend only on slices proven Complete or Superseded at the trusted pre-review `HEAD`; candidate status edits cannot manufacture readiness. Once admitted, it receives bounded orchestration priority over unrelated grabbable work instead of relying on filename order, and explicit `--slice` selection cannot bypass that barrier outside an exact trusted same-worktree repair. A late post-validation state-transition failure writes a durable failure summary and repair context, so the next bounded attempt resumes the same candidate rather than repeating the review. A later independent review must explicitly report every inherited Finding ID/Root ID pair as `Closed` before the episode is complete; any `Carried` finding continues the same episode with the complete grouped contract. A genuine later regression opens the next audited episode on those stable identities rather than restarting generation one or creating generation three. After `architecture_review_max_terminal_repair_episodes`, another independently reproduced recurrence is recorded as `Quarantined`: it remains a release blocker but no longer prevents unrelated queued slices from finishing. Every later review must retain and re-evaluate the stable quarantine; fresh bound `Closed` evidence archives and removes it, while omission or identity remapping fails closed. The loop refuses final release completion while any quarantine remains. Malformed, unauthenticated, or multi-finalizer recurrence remains fail-closed. Review agents cannot modify or widen either protected policy.
