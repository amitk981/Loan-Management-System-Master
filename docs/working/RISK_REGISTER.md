...
| R-005 | First product slice introduces backend/auth foundation and may exceed a small-diff comfort zone. | Slice 002 | High | Medium | Use `CODEX_PROFILE=deep`, require execution plan, preserve worktree isolation, and stop if scope grows beyond slice limits. | Slice 002 | Open |
