...
|---|---|---|---|
| `Dashboard` | `GET /api/v1/dashboard/` | API-backed shell | Renders backend `role_context`, `cards[]`, and the caller-scoped 012EA `tasks[]` summary from the same selector used by Task Inbox. |
...
| `TaskInbox` | `GET /api/v1/tasks/`; task reassign/comment/block action routes | API-backed staff inbox | Uses strict backend pagination and every source-listed S03 filter, renders the complete task column projection, opens the linked application/loan, and surfaces backend action denial. Reassign is additionally gated by `users.team.manage`; optional row `available_actions` narrows visible actions. No task export control is shown because no task export route/report code exists. |
...
| `AuditorEpic011View` | `GET /api/v1/auditor/epic-011/` | API-backed read-only audit view | 011O composes existing card, filter, badge, table, and detail patterns for Internal Auditor only. It exposes action-free default/recovery, closure/archive, compliance/KYC/evidence, and grievance truth with immutable references and explicit loading/empty/error/unauthorised states; 012DAC retains it under a dedicated Audit & Archive auditor-records tab while making S74 the default view. |
| `AuditArchiveHub` S74 | `GET /api/v1/audit-logs/`; `GET/POST /api/v1/audit-observations/`; `GET /api/v1/audit-observations/{id}/` | API-backed read-only explorer and immutable observations | 012DAC makes S74 the default Audit & Archive view for authorised staff, with backend filters/pagination, a strict audit-field render whitelist, no audit mutations, and truthful loading/empty/error/unauthorised states. Scoped Internal Auditors may create and revisit separate immutable observations against an explicitly selected audit row; other roles see no observation form. The retained archive register and 011 auditor evidence view remain reachable through existing tabs. |
...
| `MP04_MyProfile` | `GET /api/v1/portal/profile/`; portal KYC correction list/evidence/submit routes | API-backed masked profile and correction requests | Reuses Epic 004 member subresources for profile, nominees, shareholding, land/crop, KYC, bank accounts, and cancelled cheques. PAN/Aadhaar/bank values stay masked; no portal reveal control exists. The 011M2 panel submits self-attested evidence and shows borrower-safe correction status/dates/rejection reasons without internal review notes. |
