...
- Roles with no seeded production permission links, including the A-007 `sales_team_user`, `it_head`, and `management_viewer` cases, return an empty permission list until source documents define grants. The guarded local/demo `seed_demo_users` command is the narrow exception: in a local demo database it grants `tracer.lifecycle.run` to the demo tracer user's existing `sales_team_user` role per A-011.
