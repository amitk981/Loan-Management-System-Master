...
For local demonstrations, deterministic staff demo users are available only while the
central `ENABLE_DEMO_SURFACES` development setting is enabled and after the guarded seed
command is run with both local flags enabled:
...
| Local demo staff seed | Implemented in slice 002K; corrected in 002K2; production-isolated in 012E2 | Login, dashboard smoke, admin/tracer permission smoke | `implementation-roadmap.md` §10, §20-22; `technical-architecture.md` §8-12, §17-18; `auth-permissions.md`; `api-contracts.md` §11-12, §43-44 | `python manage.py seed_demo_users` is a guarded local/dev seed path. It refuses unless the centralized deployment setting enables demo surfaces and both `SFPCL_DEBUG=true` and `SFPCL_ALLOW_DEMO_SEED=true`; production settings refuse this command, `seed_e2e_users`, `seed_portal_e2e_fixture`, `seed_epic_009_e2e_fixture`, and `seed_approval_configuration` even if callers force their command flags. Production authentication also rejects any pre-existing synthetic `@sfpcl.example` staff or portal identity before creating a session, so promotion of a contaminated local database cannot reactivate predictable credentials. The command calls `seed_catalogue()`, creates or updates deterministic `demo.*@sfpcl.example` staff users with active primary roles and memberships, and does not alter `e2e.*` users. Demo users authenticate through the real `/auth/login/` and `/auth/me/` endpoints; there is no demo auth bypass. The zero-permission user returns `permissions: []` and `available_actions: []`; the tracer-only user uses the guarded local/dev-only `local_demo_tracer_user` role and returns only `tracer.lifecycle.run`; the shared source-catalogue `sales_team_user` role remains permission-neutral until source documents define grants; system admin preserves canonical action-specific user-admin permissions without broad `manage_users` aliases. |
...
The 002EX tracer is a deliberately thin development integration proof, not the final
member/application/finance contract. Production settings do not install its Django app
or register its routes. When the centralized non-production setting enables it, the
tracer follows the source API rules for versioning, envelopes, explicit action endpoints,
backend-enforced transitions, and audit/workflow observability.
