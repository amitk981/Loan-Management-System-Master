...
from sfpcl_credit.documents.models import DocumentFile, DocumentTemplate, LoanDocument
...
def can_generate(user):
    return GENERATE_PERMISSION in auth_service.effective_permission_codes(user)
...
def list_for_application(*, application_id, query_params):
    ...
    queryset = LoanDocument.objects.select_related("document", "document_template").filter(
    ...
    return [serialize_metadata(row) for row in rows], {
        "page": page,
        "page_size": page_size,
