...
from sfpcl_credit.applications.modules.document_checklist_facts import resolve_blank_cheque_bank_fact
...
def create_request(*, actor, application_id, payload, request, storage=None):
    ...
    try:
        with transaction.atomic():
            actor = User.objects.select_for_update().select_related("primary_role").filter(pk=actor.pk).first()
            permissions = set(auth_service.effective_permission_codes(actor)) if actor is not None else set()
            roles = set(auth_service.effective_role_codes(actor)) if actor is not None else set()
            if actor is None or not actor.can_authenticate() or CREATE_PERMISSION not in permissions or "credit_manager" not in roles:
                raise DomainPermissionDenied("You do not have SAP request create permission.")
            application, access = application_authority.resolve_application_access(application_id=application_id, actor=actor, required_permission=CREATE_PERMISSION, actor_permissions=permissions)
            if application is None or not access.allowed:
                raise DomainObjectAccessDenied(access)
            application = LoanApplication.objects.select_for_update().select_related("member").get(pk=application.pk)
            member = application.member
            replay = SapCustomerProfileRequest.objects.select_for_update().select_related("assigned_to_user").filter(loan_application=application, request_status__in=SapCustomerProfileRequest.ACTIVE_STATUSES).first()
            ...
            upload = SimpleUploadedFile(f"annexure-i-{application.application_reference_number}.xlsx", workbook, content_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
            ...
            row = SapCustomerProfileRequest.objects.create(
                sap_customer_profile_request_id=request_id, loan_application=application, member=member, requested_by_user=actor, assigned_to_user=assignee, sanction_decision_id_snapshot=decision.pk, sanction_approval_case_id_snapshot=decision.approval_case_id, excel_file=document, **facts
            )
            safe_evidence = {
            ...
            AuditLog.objects.create(actor_user=actor, actor_type="user", action="finance.sap_customer_code.requested", entity_type="sap_customer_profile_request", entity_id=row.pk, old_value_json=None, new_value_json=safe_evidence, ip_address=request_ip(request), user_agent=request_user_agent(request))
...
def serialize_request(row):
    return {"sap_customer_profile_request_id": str(row.pk), "request_status": row.request_status, "excel_file_id": str(row.excel_file_id), "assigned_to_user": {"user_id": str(row.assigned_to_user_id), "full_name": row.assigned_to_user.full_name}}
def _parse_payload(payload):
    ...
    try:
        ...
        raise ValidationError({"assigned_to_user_id": "Must be a valid UUID."}) from exc
    return {"assigned_to_user_id": assignee_id}
def _active_assignee(user_id):
    assignee = User.objects.select_for_update().select_related("primary_role").filter(pk=user_id, status=User.ACTIVE_STATUS, primary_role__role_code="senior_manager_finance", primary_role__status="active").first()
    if assignee is None:
        raise ValidationError({"assigned_to_user_id": "Select an active Senior Manager Finance user."})
    return assignee
def current_terminal_sanction(application):
    latest_case = ApprovalCase.objects.select_for_update().filter(loan_application=application).order_by("-cycle_number", "-submitted_at", "-approval_case_id").first()
    decision = SanctionDecision.objects.select_for_update().filter(loan_application=application, approval_case=latest_case, decision="sanctioned").first() if latest_case else None
    if application.application_status != LoanApplication.STATUS_APPROVED_BY_SANCTION or latest_case is None or latest_case.current_status != ApprovalCase.STATUS_APPROVED or decision is None or decision.sanctioned_amount is None or decision.sanctioned_amount <= Decimal("0.00") or decision.recorded_at is None:
        raise DomainInvalidStateError("A current terminal sanctioned decision is required.")
    return decision
def _canonical_facts(application, member, decision):
    ...
    address = ", ".join(part.strip() for part in (member.registered_address_line1, member.registered_address_line2, member.registered_village_city, member.registered_district, member.registered_state, member.registered_pincode) if part and part.strip())
    return {
        "farmer_full_name": member.legal_name.strip(),
        "borrower_type": member.member_type,
        "folio_number": member.folio_number.strip(),
        "aadhaar_number_encrypted": (FieldEncryption.encrypt("finance.sap_request.aadhaar", aadhaar) if aadhaar else ""),
...
def _workbook_values(facts):
    values = (
        facts["loan_application_number"],
        facts["farmer_full_name"],
        facts["borrower_type"],
        facts.pop("_plain_aadhaar"),
        facts.pop("_plain_pan"),
        facts["address_text"],
        facts["email_id"],
        facts["mobile_number"],
        facts["folio_number"],
        f'{facts["sanctioned_amount"]:.2f}',
        facts["sanction_date"].isoformat(),
        facts["bank_account_last4"],
        facts["ifsc"],
    )
    return values
