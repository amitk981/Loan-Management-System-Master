...
class PowerOfAttorney(models.Model):
    ...
    loan_document = models.ForeignKey(
        LoanDocument, on_delete=models.PROTECT, related_name="powers_of_attorney"
    )
    stamp_duty_record = models.ForeignKey(
        StampDutyRecord, on_delete=models.PROTECT, related_name="powers_of_attorney"
    )
    notarisation_record = models.ForeignKey(
        NotarisationRecord, on_delete=models.PROTECT, related_name="powers_of_attorney"
    )
    execution_status = models.CharField(max_length=60, db_index=True)
