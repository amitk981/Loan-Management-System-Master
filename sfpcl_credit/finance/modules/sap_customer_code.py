...
def send_request(*, actor, request_id, payload, request, storage=None):
    ...
    with transaction.atomic():
        ...
        row, application, member, assignee = _locked_request_scope(
            request_id, actor_id=actor.pk, owner_field="requested_by_user_id"
        )
        ...
        _record_audit(
            ...
            action="finance.sap_customer_code.sent", evidence=evidence,
        )
...
def complete_request(*, actor, request_id, payload, request):
    ...
    with transaction.atomic():
        ...
        row, application, member, assignee = _locked_request_scope(
            request_id, actor_id=actor.pk, owner_field="assigned_to_user_id"
        )
        _require_current_scope(row, application, member, assignee)
        ...
        confirmation_document = _confirmation_document(
            actor=actor, row=row, document_id=values["confirmation_document_id"]
        )
        active_code = (
            SapCustomerCode.objects.select_for_update()
            .filter(member=member, status=SapCustomerCode.STATUS_ACTIVE)
            .first()
        )
        inactive_member_code_exists = (
            SapCustomerCode.objects.select_for_update()
            .filter(member=member, status=SapCustomerCode.STATUS_INACTIVE)
            .exists()
        )
        code_owner = (
            SapCustomerCode.objects.select_for_update()
            .filter(sap_customer_code__iexact=values["sap_customer_code"])
            .first()
        )
        ...
        if active_code is None and inactive_member_code_exists:
            raise SapRequestConflict(
                "This member has inactive SAP code history requiring governed correction."
            )
        ...
        if active_code is not None:
            ...
            if origin is not None and origin.completed_at > row.created_at:
                raise SapRequestConflict(
                    "Another request for this member completed with the active SAP code."
                )
            code = active_code
        else:
            try:
                ...
                raise SapRequestConflict(
                    "The SAP customer code or active member code already exists."
                ) from exc
...
def read_member_code(*, actor, member_id):
    with transaction.atomic():
        ...
        member = (
            ...
            .filter(pk=member_id, is_deleted=False, membership_status="active")
            .first()
        )
        if member is None:
...
def serialize_completed_request(row):
    ...
    return {
        ...
        "confirmation_document": (
            {
                "document_id": str(code.confirmation_document_id),
                "file_name": code.confirmation_document.file_name,
                "mime_type": code.confirmation_document.mime_type,
                "sensitivity_level": code.confirmation_document.sensitivity_level,
            }
            if code.confirmation_document_id else None
        ),
    }
...
def _locked_actor(actor, permission, role):
    persisted = (
        User.objects.select_for_update().select_related("primary_role")
        .filter(pk=getattr(actor, "pk", None)).first()
    )
    permissions = (
        set(auth_service.effective_permission_codes(persisted)) if persisted else set()
    )
    roles = set(auth_service.effective_role_codes(persisted)) if persisted else set()
    if (
        persisted is None or not persisted.can_authenticate()
        or permission not in permissions or role not in roles
    ):
...
def _locked_request_scope(request_id, *, actor_id, owner_field):
    ...
    if reference is None or reference[owner_field] != actor_id:
        raise DomainObjectAccessDenied(None)
    application = (
        LoanApplication.objects.select_for_update().select_related("member")
        .filter(pk=reference["loan_application_id"]).first()
    )
    member = (
        Member.objects.select_for_update().filter(pk=reference["member_id"]).first()
    )
    if application is None or member is None:
        raise DomainObjectAccessDenied(None)
    current_terminal_sanction(application)
    row = (
        SapCustomerProfileRequest.objects.select_for_update(of=("self",))
        .select_related(
            "assigned_to_user__primary_role", "excel_file",
            "sent_communication", "sent_task", "sap_customer_code__confirmation_document",
        )
        .get(pk=request_id)
    )
    assignee = (
        User.objects.select_for_update().select_related("primary_role")
        .filter(pk=row.assigned_to_user_id).first()
    )
    if assignee is None:
        raise DomainObjectAccessDenied(None)
...
def _require_current_scope(row, application, member, assignee):
    ...
    if (
        ...
        raise DomainInvalidStateError(
            "The SAP request is no longer bound to current terminal sanction evidence."
        )
...
def _confirmation_document(*, actor, row, document_id):
    ...
    try:
        ...
        raise ValidationError(
            {"confirmation_document_id": _INACCESSIBLE_DOCUMENT}
        ) from exc
    allowed_scope = (
        (
            provenance.related_entity_type == "sap_customer_profile_request"
            and provenance.related_entity_id == row.pk
        )
        or (
            provenance.related_entity_type == "loan_application"
            and provenance.related_entity_id == row.loan_application_id
        )
    )
    if (
        provenance.document_category != "sap_confirmation"
        or not allowed_scope
        or provenance.document.uploaded_by_user_id != actor.pk
        or provenance.document.sensitivity_level != DocumentFile.SENSITIVITY_RESTRICTED
    ):
        raise ValidationError(
            {"confirmation_document_id": _INACCESSIBLE_DOCUMENT}
        )
...
def _require_reuse_facts(code, values, document):
    comparisons = (
        ...
        (document.pk if document else None, code.confirmation_document_id),
...
def _require_exact_completion_replay(row, values):
    ...
    if (
        ...
        or code.created_at_sap != values["created_at_sap"]
        or code.confirmation_document_id != values["confirmation_document_id"]
        or code.confirmation_notes != values["confirmation_notes"]
    ):
        raise SapRequestConflict("The completed SAP request facts cannot be changed.")
...
def _parse_complete_payload(payload):
    if not isinstance(payload, dict):
        raise ValidationError({"body": "A JSON object is required."})
    allowed = {
        "sap_customer_code", "sap_vendor_code", "created_at_sap",
        "confirmation_document_id", "confirmation_notes",
    }
    errors = {field: "Unknown field." for field in sorted(set(payload) - allowed)}
    customer_code = _code_value(
        payload.get("sap_customer_code"), "sap_customer_code", required=True,
        errors=errors,
    )
    vendor_code = _code_value(
        payload.get("sap_vendor_code"), "sap_vendor_code", required=False,
        errors=errors,
    )
    created_at_sap = None
