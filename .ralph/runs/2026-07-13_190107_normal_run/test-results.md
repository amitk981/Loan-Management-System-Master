# test Results

Command: npm test --if-present

Node PATH pin: /Users/amitkallapa/.nvm/versions/node/v18.20.4/bin


> sfpcl-lms@1.0.0 test
> vitest run

[33mThe CJS build of Vite's Node API is deprecated. See https://vite.dev/guide/troubleshooting.html#vite-cjs-node-api-deprecated for more details.[39m

 RUN  v3.2.6 /Users/amitkallapa/LMS/.ralph/worktrees/2026-07-13_190107_normal_run/sfpcl-lms

 ✓ src/pages/members/MemberProfile.container.test.tsx (4 tests) 311ms
 ✓ src/pages/borrower/portal/applications/MP05_NewApplication.test.tsx (7 tests) 1034ms
   ✓ MP05 loan-limit display authority (006Z2 interim regression) > shows independent 400, 403, and 409 errors without retry or projection refetch  587ms
 ✓ src/pages/appraisal/AppraisalWorkbench.test.tsx (22 tests) 173ms
 ✓ src/pages/applications/WitnessPanel.container.test.tsx (10 tests) 1196ms
   ✓ mounted witness resource actions > captures with the exact body and refetches the canonical collection once  417ms
 ✓ src/pages/members/Borrower360.test.tsx (4 tests) 101ms
 ✓ src/pages/appraisal/AppraisalWorkbench.container.test.tsx (14 tests) 1709ms
   ✓ default AppraisalWorkbench authenticated HTTP container > calculates a limit from entered source IDs and refreshes four reads  317ms
   ✓ default AppraisalWorkbench authenticated HTTP container > posts the rejected Credit Manager decision once  308ms
 ✓ src/pages/members/MemberProfile.test.tsx (26 tests) 260ms
 ✓ src/pages/applications/CompletenessWorkbench.test.tsx (5 tests) 107ms
 ✓ src/pages/members/MemberGovernanceForm.test.tsx (5 tests) 2180ms
   ✓ MemberGovernanceForm > submits the institution create variant without individual identity/profile fields  1109ms
   ✓ MemberGovernanceForm > submits every individual registration profile field  835ms
 ✓ src/pages/applications/ApplicationDetail.test.tsx (9 tests) 100ms
 ✓ src/pages/borrower/portal/PortalMemberViews.test.tsx (4 tests) 67ms
 ✓ src/pages/Dashboard.test.tsx (13 tests) 66ms
 ✓ src/pages/members/MemberDirectory.test.tsx (5 tests) 57ms
 ✓ src/services/authSession.test.ts (15 tests) 20ms
 ✓ src/pages/notifications/NotificationsCenter.test.tsx (6 tests) 49ms
 ✓ src/pages/applications/ApplicationList.test.tsx (2 tests) 48ms
 ✓ src/pages/borrower/portal/auth/MP00_Login.test.tsx (2 tests) 23ms
 ✓ src/services/navigationPermissions.test.ts (7 tests) 7ms
 ✓ src/services/applicationIntakeApi.test.ts (5 tests) 16ms
 ✓ src/services/creditAssessmentApi.test.ts (5 tests) 18ms
 ✓ src/services/portalApi.test.ts (4 tests) 8ms
 ✓ src/pages/sanction/SanctionWorkbench.test.tsx (2 tests) 12ms
 ✓ src/services/tracerApi.test.ts (2 tests) 6ms
 ✓ src/contexts/RoleContext.test.tsx (2 tests) 11ms
 ✓ src/pages/profile/MyProfile.test.tsx (1 test) 15ms
 ✓ src/services/demoAuthFlag.test.tsx (3 tests) 4082ms
   ✓ VITE_ENABLE_DEMO_AUTH real App/RoleProvider boundary (005FA4) > fails closed when the flag is unset  3530ms
 ✓ src/pages/applications/NomineeSelectionViews.test.tsx (3 tests) 9ms
 ✓ src/utils/applicationDisplay.test.ts (5 tests) 2ms
 ✓ src/pages/members/MemberGovernanceForm.container.test.tsx (16 tests) 2979ms
   ✓ MemberGovernanceForm production container > routes Directory registration into canonical Profile readback with the exact create ledger  1803ms

 Test Files  29 passed (29)
      Tests  208 passed (208)
   Start at  19:28:35
   Duration  6.75s (transform 5.47s, setup 0ms, collect 8.79s, tests 14.66s, environment 3.03s, prepare 1.92s)

