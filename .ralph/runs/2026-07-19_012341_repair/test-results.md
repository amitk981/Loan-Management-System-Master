# test Results

Command: npm test --if-present

Node PATH pin: /Users/amitkallapa/.nvm/versions/node/v20.19.6/bin


> sfpcl-lms@1.0.0 test
> vitest run

[33mThe CJS build of Vite's Node API is deprecated. See https://vite.dev/guide/troubleshooting.html#vite-cjs-node-api-deprecated for more details.[39m

 RUN  v3.2.6 /Users/amitkallapa/LMS/.ralph/worktrees/2026-07-19_003418_normal_run/sfpcl-lms

 ✓ src/pages/applications/WitnessPanel.container.test.tsx (10 tests) 1477ms
   ✓ mounted witness resource actions > captures with the exact body and refetches the canonical collection once  552ms
 ✓ src/pages/settings/SettingsHub.test.tsx (10 tests) 1511ms
   ✓ SettingsHub Approval Matrix panel > permits a canonical manager to submit a complete successor version as a pending proposal  664ms
   ✓ SettingsHub remaining panels > creates a complete successor as a separate audited draft for a canonical policy manager  471ms
 ✓ src/pages/appraisal/AppraisalWorkbench.container.test.tsx (14 tests) 2159ms
   ✓ default AppraisalWorkbench authenticated HTTP container > calculates a limit from entered source IDs and refreshes four reads  368ms
 ✓ src/pages/members/MemberGovernanceForm.test.tsx (5 tests) 2554ms
   ✓ MemberGovernanceForm > submits the institution create variant without individual identity/profile fields  1338ms
   ✓ MemberGovernanceForm > submits every individual registration profile field  955ms
 ✓ src/pages/documentation/DocumentationHub.test.tsx (18 tests) 2777ms
   ✓ 008M2 documentation workspace contract > posts a server-owned approval and refetches once without optimism  614ms
   ✓ 008M2 documentation workspace contract > keeps a conflict visible with no optimistic change, retry, or refetch  454ms
   ✓ 008M2 documentation workspace contract > submits a signed-copy upload through the opaque action and refetches once  321ms
 ✓ src/pages/borrower/portal/applications/MP05_NewApplication.test.tsx (7 tests) 1222ms
   ✓ MP05 loan-limit display authority (006Z2 interim regression) > creates, submits exactly once, and canonically refetches the returned amount  314ms
   ✓ MP05 loan-limit display authority (006Z2 interim regression) > shows independent 400, 403, and 409 errors without retry or projection refetch  628ms
 ✓ src/pages/registers/RegistersHub.test.tsx (8 tests) 979ms
   ✓ RegistersHub owned approval register panels > renders only the server-scoped frozen sanction page and replaces pagination after a filter change  392ms
   ✓ RegistersHub owned approval register panels > applies only canonical financial-year values  330ms
 ✓ src/pages/borrower/portal/documents/PortalDocumentationActions.test.tsx (8 tests) 632ms
   ✓ member portal documentation actions > renders server-owned actions and performs one canonical refetch after upload  408ms
 ✓ src/pages/borrower/portal/applications/MP11_DeficiencyResponse.test.tsx (2 tests) 556ms
   ✓ MP11 deficiency response > uploads a server-contracted response, refetches canonical state, and resubmits  462ms
 ✓ src/pages/borrower/portal/disbursement/MP14_DisbursementStatus.test.tsx (4 tests) 298ms
 ✓ src/pages/borrower/portal/PortalMemberViews.test.tsx (6 tests) 179ms
 ✓ src/pages/members/MemberProfile.container.test.tsx (4 tests) 381ms
 ✓ src/pages/appraisal/AppraisalWorkbench.test.tsx (22 tests) 184ms
 ✓ src/pages/members/MemberProfile.test.tsx (26 tests) 228ms
 ✓ src/pages/applications/CompletenessWorkbench.test.tsx (5 tests) 62ms
 ✓ src/services/authSession.test.ts (36 tests) 41ms
 ✓ src/pages/members/Borrower360.test.tsx (4 tests) 105ms
 ✓ src/pages/applications/ApplicationDetail.test.tsx (9 tests) 87ms
 ✓ src/services/portalApi.test.ts (7 tests) 19ms
 ✓ src/pages/notifications/NotificationsCenter.test.tsx (6 tests) 30ms
 ✓ src/pages/Dashboard.test.tsx (13 tests) 44ms
 ✓ src/pages/applications/ApplicationList.test.tsx (2 tests) 35ms
 ✓ src/pages/members/MemberDirectory.test.tsx (5 tests) 49ms
 ✓ src/services/approvalRegistersApi.test.ts (5 tests) 14ms
 ✓ src/pages/sanction/SanctionWorkbench.test.tsx (39 tests) 6199ms
   ✓ SanctionWorkbench authenticated container > renders the authoritative total and replaces the queue from the next server page  446ms
   ✓ SanctionWorkbench authenticated container > keeps a newer filter authoritative while an action detail refresh finishes  499ms
   ✓ SanctionWorkbench authenticated container > keeps a newer empty filter authoritative while action submission finishes  336ms
   ✓ SanctionWorkbench authenticated container > keeps a newer malformed state authoritative when an action detail refresh fails later  332ms
   ✓ SanctionWorkbench authenticated container > keeps a newer empty state authoritative when an action detail refresh fails later  376ms
   ✓ SanctionWorkbench authenticated container > keeps a newer filter authoritative while an action decision refresh finishes  381ms
   ✓ SanctionWorkbench authenticated container > loads frozen case truth and approves through the exact case boundary before canonical refresh  336ms
   ✓ SanctionWorkbench authenticated container > requires a reason and sends the exact Reject request  404ms
   ✓ SanctionWorkbench authenticated container > requires a reason and sends the exact Return for Clarification request  422ms
   ✓ SanctionWorkbench authenticated container > uploads three application-scoped legal files before recording bounded special-case evidence  853ms
 ✓ src/pages/applications/NomineeSelectionViews.test.tsx (3 tests) 16ms
 ✓ src/services/creditAssessmentApi.test.ts (5 tests) 15ms
 ✓ src/pages/profile/MyProfile.test.tsx (1 test) 16ms
 ✓ src/services/navigationPermissions.test.ts (9 tests) 16ms
 ✓ src/pages/borrower/portal/auth/MP00_Login.test.tsx (2 tests) 11ms
 ✓ src/contexts/RoleContext.test.tsx (2 tests) 16ms
 ✓ src/services/applicationIntakeApi.test.ts (5 tests) 7ms
 ✓ src/services/loanPolicyApi.test.ts (2 tests) 7ms
 ✓ src/services/tracerApi.test.ts (2 tests) 5ms
 ✓ src/utils/applicationDisplay.test.ts (5 tests) 3ms
 ✓ src/services/demoAuthFlag.test.tsx (3 tests) 3203ms
   ✓ VITE_ENABLE_DEMO_AUTH real App/RoleProvider boundary (005FA4) > fails closed when the flag is unset  2704ms
   ✓ VITE_ENABLE_DEMO_AUTH real App/RoleProvider boundary (005FA4) > fails closed when the flag is explicit false  342ms
 ✓ src/playwrightBrowser.test.ts (4 tests) 2ms
 ✓ src/pages/members/MemberGovernanceForm.container.test.tsx (16 tests) 5350ms
   ✓ MemberGovernanceForm production container > routes Directory registration into canonical Profile readback with the exact create ledger  2707ms
   ✓ MemberGovernanceForm production container > posts only the protected identity delta through the shared HTTP transport  388ms

 Test Files  38 passed (38)
      Tests  334 passed (334)
   Start at  01:29:30
   Duration  9.03s (transform 5.47s, setup 0ms, collect 13.41s, tests 30.49s, environment 5.69s, prepare 2.93s)


Duration milliseconds: 9554
Exit code: 0
