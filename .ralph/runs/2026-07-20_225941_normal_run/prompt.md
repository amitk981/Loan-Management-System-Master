...
- This corrective slice has a machine-readable closure contract. Before returning, run exactly: ./scripts/ralph-validate-review-closure.sh --slice docs/slices/010H3-interest-policy-and-reclassification-integrity-closure.md --run-dir .ralph/runs/2026-07-20_225941_normal_run . Keep rerunning that fast check until it prints PASS. Markdown prose may follow a table only after a blank line; Python tests may use an exact path::selector or exact Django dotted test label. Do not defer a failure from this command to the orchestrator.
