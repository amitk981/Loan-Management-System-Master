# test Results

Command: npm test --if-present

Node PATH pin: /Users/amitkallapa/.nvm/versions/node/v20.19.6/bin


> sfpcl-lms@1.0.0 test
> vitest run

[33mThe CJS build of Vite's Node API is deprecated. See https://vite.dev/guide/troubleshooting.html#vite-cjs-node-api-deprecated for more details.[39m

 RUN  v3.2.6 /Users/amitkallapa/LMS/.ralph/worktrees/2026-07-25_003342_normal_run/sfpcl-lms

 ✓ src/pages/settings/SettingsHub.test.tsx (10 tests) 1269ms
   ✓ SettingsHub Approval Matrix panel > permits a canonical manager to submit a complete successor version as a pending proposal  601ms
   ✓ SettingsHub remaining panels > creates a complete successor as a separate audited draft for a canonical policy manager  320ms
 ✓ src/pages/repayments/RepaymentsHub.test.tsx (9 tests) 1639ms
   ✓ 010MA Repayments Hub wiring > posts one governed direct attempt, displays backend allocation, and refreshes reads  530ms
   ✓ 010MA Repayments Hub wiring > shows backend posting denial without retry or canonical refresh: AuthSessionError: Amount must be a positive decimal.  338ms
   ✓ 010MA Repayments Hub wiring > shows backend posting denial without retry or canonical refresh: AuthSessionError: This bank reference is already recorded.  346ms
 ✓ src/pages/appraisal/AppraisalWorkbench.container.test.tsx (14 tests) 2009ms
   ✓ default AppraisalWorkbench authenticated HTTP container > calculates a limit from entered source IDs and refreshes four reads  419ms
   ✓ default AppraisalWorkbench authenticated HTTP container > posts the rejected Credit Manager decision once  343ms
 ✓ src/pages/members/MemberGovernanceForm.test.tsx (5 tests) 2338ms
   ✓ MemberGovernanceForm > submits the institution create variant without individual identity/profile fields  1231ms
   ✓ MemberGovernanceForm > submits every individual registration profile field  814ms
 ✓ src/pages/documentation/DocumentationHub.test.tsx (18 tests) 2694ms
   ✓ 008M2 documentation workspace contract > posts a server-owned approval and refetches once without optimism  518ms
   ✓ 008M2 documentation workspace contract > keeps a conflict visible with no optimistic change, retry, or refetch  404ms
   ✓ 008M2 documentation workspace contract > submits a signed-copy upload through the opaque action and refetches once  357ms
 ✓ src/pages/tasks/TaskInbox.test.tsx (19 tests) 1512ms
   ✓ Task Inbox screen > renders the S03 API columns and replaces the page through backend pagination  492ms
   ✓ Task Inbox screen > opens the linked application and completes a permitted comment action  353ms
   ✓ Task Inbox screen > surfaces the backend rejection when task authority changes before an action  305ms
 ✓ src/pages/applications/WitnessPanel.container.test.tsx (10 tests) 1338ms
   ✓ mounted witness resource actions > captures with the exact body and refetches the canonical collection once  427ms
 ✓ src/pages/defaults/DefaultRecoveryHub.test.tsx (11 tests) 1379ms
   ✓ 011PA default case and frozen-note read surface > enforces a reason, posts the server-fixed action, and refetches canonical terminal state  530ms
 ✓ src/pages/disbursement/PaymentAuthorisationHub.test.tsx (5 tests) 1100ms
   ✓ 009K CFC payment authorisation workspace > shows CFC actions only when the backend projects them and posts the decision reason  373ms
   ✓ 009K CFC payment authorisation workspace > surfaces backend duplicate UTR and permission errors without optimistic success  446ms
 ✓ src/pages/closure/LoanClosureHub.test.tsx (6 tests) 1954ms
   ✓ 011PC closure readiness and downstream owner wiring > validates notes, closes from server readiness, and refetches canonical downstream reads  395ms
   ✓ 011PC closure readiness and downstream owner wiring > issues NOC then renders the canonical refetch projection  553ms
   ✓ 011PC closure readiness and downstream owner wiring > records server-owned security state and archives only after downstream prerequisites  657ms
 ✓ src/pages/sanction/SanctionWorkbench.test.tsx (39 tests) 5639ms
   ✓ SanctionWorkbench authenticated container > renders the authoritative total and replaces the queue from the next server page  511ms
   ✓ SanctionWorkbench authenticated container > keeps a newer filter authoritative while an action detail refresh finishes  336ms
   ✓ SanctionWorkbench authenticated container > keeps a newer empty filter authoritative while action submission finishes  321ms
   ✓ SanctionWorkbench authenticated container > keeps a newer empty state authoritative when an action detail refresh fails later  340ms
   ✓ SanctionWorkbench authenticated container > keeps a newer filter authoritative while an action decision refresh finishes  321ms
   ✓ SanctionWorkbench authenticated container > loads frozen case truth and approves through the exact case boundary before canonical refresh  362ms
   ✓ SanctionWorkbench authenticated container > requires a reason and sends the exact Reject request  420ms
   ✓ SanctionWorkbench authenticated container > requires a reason and sends the exact Return for Clarification request  357ms
   ✓ SanctionWorkbench authenticated container > uploads three application-scoped legal files before recording bounded special-case evidence  631ms
 ✓ src/pages/registers/RegistersHub.test.tsx (8 tests) 882ms
   ✓ RegistersHub owned approval register panels > renders only the server-scoped frozen sanction page and replaces pagination after a filter change  333ms
 ✓ src/pages/borrower/portal/applications/MP05_NewApplication.test.tsx (7 tests) 1295ms
   ✓ MP05 loan-limit display authority (006Z2 interim regression) > creates, submits exactly once, and canonically refetches the returned amount  321ms
   ✓ MP05 loan-limit display authority (006Z2 interim regression) > shows independent 400, 403, and 409 errors without retry or projection refetch  739ms
 ✓ src/pages/borrower/portal/PortalCommunicationsViews.test.tsx (5 tests) 863ms
   ✓ MP19-MP24 member communication views > renders all MP24 guide sections, validates required fields, submits, and shows resolution  649ms
 ✓ src/pages/search/GlobalSearchResults.test.tsx (5 tests) 651ms
 ✓ src/pages/borrower/portal/documents/PortalDocumentationActions.test.tsx (8 tests) 669ms
   ✓ member portal documentation actions > renders server-owned actions and performs one canonical refetch after upload  448ms
 ✓ src/pages/borrower/portal/applications/MP11_DeficiencyResponse.test.tsx (2 tests) 477ms
   ✓ MP11 deficiency response > uploads a server-contracted response, refetches canonical state, and resubmits  400ms
 ✓ src/pages/members/MemberProfile.container.test.tsx (4 tests) 357ms
 ✓ src/components/layout/Header.notifications.test.tsx (9 tests) 533ms
 ✓ src/pages/servicing/InterestMonitoringWorkspaces.test.tsx (6 tests) 483ms
 ✓ src/pages/members/MemberGovernanceForm.container.test.tsx (16 tests) 5230ms
   ✓ MemberGovernanceForm production container > routes Directory registration into canonical Profile readback with the exact create ledger  2805ms
   ✓ MemberGovernanceForm production container > performs one ordinary human-like update before canonical Profile readback with the exact update ledger  314ms
   ✓ MemberGovernanceForm production container > posts only the protected identity delta through the shared HTTP transport  305ms
 ✓ src/pages/disbursement/DisbursementHub.test.tsx (4 tests) 589ms
   ✓ 009K disbursement finance workspace > submits Money fields with one stable key and treats replay as success  344ms
 ✓ src/pages/loan-accounts/LoanAccount360.test.tsx (6 tests) 445ms
 ✓ src/pages/Dashboard.test.tsx (24 tests) 315ms
 ✓ src/pages/borrower/portal/disbursement/MP14_DisbursementStatus.test.tsx (6 tests) 299ms
 ✓ src/pages/borrower/portal/loans/PortalLoanViews.test.tsx (6 tests) 365ms
 ✓ src/pages/borrower/portal/PortalMemberViews.test.tsx (7 tests) 246ms
 ✓ src/pages/compliance/AuditorEpic011View.test.tsx (4 tests) 255ms
 ✓ src/components/layout/Header.search.test.tsx (1 test) 225ms
 ✓ src/services/productionSurfaceIsolation.test.ts (1 test) 156ms
 ✓ src/pages/members/Borrower360.test.tsx (4 tests) 70ms
 ✓ src/pages/applications/ApplicationDetail.test.tsx (9 tests) 61ms
 ✓ src/pages/members/MemberProfile.test.tsx (26 tests) 155ms
 ✓ src/services/authSession.test.ts (39 tests) 33ms
 ✓ src/pages/appraisal/AppraisalWorkbench.test.tsx (22 tests) 186ms
 ✓ src/pages/applications/CompletenessWorkbench.test.tsx (5 tests) 63ms
 ✓ src/services/servicingApi.test.ts (13 tests) 26ms
 ✓ src/pages/notifications/NotificationsCenter.test.tsx (6 tests) 40ms
 ✓ src/services/portalApi.test.ts (10 tests) 30ms
 ✓ src/pages/applications/ApplicationList.test.tsx (3 tests) 42ms
 ✓ src/pages/members/MemberDirectory.test.tsx (5 tests) 79ms
 ✓ src/services/applicationIntakeApi.test.ts (5 tests) 8ms
 ✓ src/pages/profile/MyProfile.test.tsx (1 test) 15ms
 ✓ src/pages/borrower/portal/auth/MP00_Login.test.tsx (2 tests) 10ms
 ✓ src/services/navigationPermissions.test.ts (9 tests) 8ms
 ✓ src/services/creditAssessmentApi.test.ts (5 tests) 10ms
 ✓ src/services/disbursementApi.test.ts (3 tests) 11ms
 ✓ src/pages/applications/NomineeSelectionViews.test.tsx (3 tests) 8ms
 ✓ src/services/recoveryApi.test.ts (4 tests) 7ms
 ✓ src/services/tracerApi.test.ts (2 tests) 5ms
 ✓ src/contexts/RoleContext.test.tsx (2 tests) 7ms
 ✓ src/services/approvalRegistersApi.test.ts (5 tests) 8ms
 ✓ src/services/loanPolicyApi.test.ts (2 tests) 5ms
 ✓ src/utils/applicationDisplay.test.ts (5 tests) 2ms
 ✓ src/playwright.seed.test.ts (3 tests) 2ms
 ✓ src/utils/formatMoney.test.ts (1 test) 2ms
 ✓ src/playwrightBrowser.test.ts (4 tests) 5ms
 ✓ src/services/demoAuthFlag.test.tsx (4 tests) 2473ms
   ✓ VITE_ENABLE_DEMO_AUTH real App/RoleProvider boundary (005FA4) > fails closed when the flag is unset  2285ms

 Test Files  58 passed (58)
      Tests  477 passed (477)
   Start at  02:03:44
   Duration  12.46s (transform 5.50s, setup 0ms, collect 17.72s, tests 40.57s, environment 10.50s, prepare 3.98s)


Duration milliseconds: 13028
Exit code: 0
