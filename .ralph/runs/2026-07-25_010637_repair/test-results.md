# test Results

Command: npm test --if-present

Node PATH pin: /Users/amitkallapa/.nvm/versions/node/v20.19.6/bin


> sfpcl-lms@1.0.0 test
> vitest run

[33mThe CJS build of Vite's Node API is deprecated. See https://vite.dev/guide/troubleshooting.html#vite-cjs-node-api-deprecated for more details.[39m

 RUN  v3.2.6 /Users/amitkallapa/LMS/.ralph/worktrees/2026-07-25_003342_normal_run/sfpcl-lms

 ✓ src/pages/repayments/RepaymentsHub.test.tsx (9 tests) 1577ms
   ✓ 010MA Repayments Hub wiring > posts one governed direct attempt, displays backend allocation, and refreshes reads  543ms
   ✓ 010MA Repayments Hub wiring > shows backend posting denial without retry or canonical refresh: AuthSessionError: Amount must be a positive decimal.  338ms
   ✓ 010MA Repayments Hub wiring > shows backend posting denial without retry or canonical refresh: AuthSessionError: This bank reference is already recorded.  315ms
 ✓ src/pages/appraisal/AppraisalWorkbench.container.test.tsx (14 tests) 1725ms
   ✓ default AppraisalWorkbench authenticated HTTP container > calculates a limit from entered source IDs and refreshes four reads  323ms
 ✓ src/pages/closure/LoanClosureHub.test.tsx (6 tests) 1872ms
   ✓ 011PC closure readiness and downstream owner wiring > validates notes, closes from server readiness, and refetches canonical downstream reads  526ms
   ✓ 011PC closure readiness and downstream owner wiring > issues NOC then renders the canonical refetch projection  569ms
   ✓ 011PC closure readiness and downstream owner wiring > records server-owned security state and archives only after downstream prerequisites  514ms
 ✓ src/pages/members/MemberGovernanceForm.test.tsx (5 tests) 2261ms
   ✓ MemberGovernanceForm > submits the institution create variant without individual identity/profile fields  1119ms
   ✓ MemberGovernanceForm > submits every individual registration profile field  861ms
 ✓ src/pages/documentation/DocumentationHub.test.tsx (18 tests) 2345ms
   ✓ 008M2 documentation workspace contract > posts a server-owned approval and refetches once without optimism  508ms
   ✓ 008M2 documentation workspace contract > keeps a conflict visible with no optimistic change, retry, or refetch  374ms
 ✓ src/pages/defaults/DefaultRecoveryHub.test.tsx (11 tests) 1307ms
   ✓ 011PA default case and frozen-note read surface > renders list/detail, grace, extension, and frozen note from backend projections  328ms
   ✓ 011PA default case and frozen-note read surface > enforces a reason, posts the server-fixed action, and refetches canonical terminal state  458ms
 ✓ src/pages/tasks/TaskInbox.test.tsx (19 tests) 1418ms
   ✓ Task Inbox screen > renders the S03 API columns and replaces the page through backend pagination  449ms
   ✓ Task Inbox screen > opens the linked application and completes a permitted comment action  304ms
   ✓ Task Inbox screen > surfaces the backend rejection when task authority changes before an action  302ms
 ✓ src/pages/settings/SettingsHub.test.tsx (10 tests) 1315ms
   ✓ SettingsHub Approval Matrix panel > permits a canonical manager to submit a complete successor version as a pending proposal  554ms
   ✓ SettingsHub remaining panels > creates a complete successor as a separate audited draft for a canonical policy manager  355ms
 ✓ src/pages/disbursement/PaymentAuthorisationHub.test.tsx (5 tests) 1074ms
   ✓ 009K CFC payment authorisation workspace > shows CFC actions only when the backend projects them and posts the decision reason  370ms
   ✓ 009K CFC payment authorisation workspace > surfaces backend duplicate UTR and permission errors without optimistic success  457ms
 ✓ src/pages/applications/WitnessPanel.container.test.tsx (10 tests) 1311ms
   ✓ mounted witness resource actions > captures with the exact body and refetches the canonical collection once  412ms
 ✓ src/pages/registers/RegistersHub.test.tsx (8 tests) 771ms
 ✓ src/pages/loan-accounts/LoanAccount360.test.tsx (6 tests) 448ms
 ✓ src/pages/borrower/portal/applications/MP05_NewApplication.test.tsx (7 tests) 1093ms
   ✓ MP05 loan-limit display authority (006Z2 interim regression) > shows independent 400, 403, and 409 errors without retry or projection refetch  665ms
 ✓ src/pages/sanction/SanctionWorkbench.test.tsx (39 tests) 5764ms
   ✓ SanctionWorkbench authenticated container > renders the authoritative total and replaces the queue from the next server page  444ms
   ✓ SanctionWorkbench authenticated container > keeps a newer filter authoritative while an action detail refresh finishes  379ms
   ✓ SanctionWorkbench authenticated container > keeps a newer empty filter authoritative while action submission finishes  317ms
   ✓ SanctionWorkbench authenticated container > keeps a newer empty state authoritative when an action detail refresh fails later  383ms
   ✓ SanctionWorkbench authenticated container > keeps a newer filter authoritative while an action decision refresh finishes  324ms
   ✓ SanctionWorkbench authenticated container > loads frozen case truth and approves through the exact case boundary before canonical refresh  312ms
   ✓ SanctionWorkbench authenticated container > requires a reason and sends the exact Reject request  376ms
   ✓ SanctionWorkbench authenticated container > requires a reason and sends the exact Return for Clarification request  413ms
   ✓ SanctionWorkbench authenticated container > uploads three application-scoped legal files before recording bounded special-case evidence  760ms
 ✓ src/pages/borrower/portal/PortalCommunicationsViews.test.tsx (5 tests) 791ms
   ✓ MP19-MP24 member communication views > renders all MP24 guide sections, validates required fields, submits, and shows resolution  559ms
 ✓ src/pages/borrower/portal/documents/PortalDocumentationActions.test.tsx (8 tests) 618ms
   ✓ member portal documentation actions > renders server-owned actions and performs one canonical refetch after upload  402ms
 ✓ src/pages/members/MemberGovernanceForm.container.test.tsx (16 tests) 4839ms
   ✓ MemberGovernanceForm production container > routes Directory registration into canonical Profile readback with the exact create ledger  2457ms
   ✓ MemberGovernanceForm production container > request preserves the backend 403 PERMISSION_DENIED facts after one mutation and no GET  310ms
 ✓ src/components/layout/Header.notifications.test.tsx (9 tests) 393ms
 ✓ src/pages/search/GlobalSearchResults.test.tsx (5 tests) 538ms
 ✓ src/pages/servicing/InterestMonitoringWorkspaces.test.tsx (6 tests) 506ms
 ✓ src/pages/disbursement/DisbursementHub.test.tsx (4 tests) 440ms
 ✓ src/pages/borrower/portal/applications/MP11_DeficiencyResponse.test.tsx (2 tests) 544ms
   ✓ MP11 deficiency response > uploads a server-contracted response, refetches canonical state, and resubmits  468ms
 ✓ src/pages/members/MemberProfile.container.test.tsx (4 tests) 378ms
 ✓ src/pages/borrower/portal/disbursement/MP14_DisbursementStatus.test.tsx (6 tests) 301ms
 ✓ src/pages/Dashboard.test.tsx (24 tests) 330ms
 ✓ src/pages/borrower/portal/loans/PortalLoanViews.test.tsx (6 tests) 314ms
 ✓ src/components/layout/Header.search.test.tsx (1 test) 220ms
 ✓ src/services/productionSurfaceIsolation.test.ts (1 test) 143ms
 ✓ src/pages/borrower/portal/PortalMemberViews.test.tsx (7 tests) 228ms
 ✓ src/pages/compliance/AuditorEpic011View.test.tsx (4 tests) 185ms
 ✓ src/pages/appraisal/AppraisalWorkbench.test.tsx (22 tests) 174ms
 ✓ src/pages/notifications/NotificationsCenter.test.tsx (6 tests) 30ms
 ✓ src/pages/members/Borrower360.test.tsx (4 tests) 77ms
 ✓ src/pages/applications/CompletenessWorkbench.test.tsx (5 tests) 54ms
 ✓ src/pages/applications/ApplicationDetail.test.tsx (9 tests) 67ms
 ✓ src/pages/members/MemberProfile.test.tsx (26 tests) 158ms
 ✓ src/services/portalApi.test.ts (10 tests) 23ms
 ✓ src/services/servicingApi.test.ts (13 tests) 33ms
 ✓ src/services/authSession.test.ts (39 tests) 39ms
 ✓ src/services/recoveryApi.test.ts (4 tests) 6ms
 ✓ src/pages/members/MemberDirectory.test.tsx (5 tests) 28ms
 ✓ src/pages/applications/ApplicationList.test.tsx (3 tests) 32ms
 ✓ src/services/creditAssessmentApi.test.ts (5 tests) 8ms
 ✓ src/services/applicationIntakeApi.test.ts (5 tests) 7ms
 ✓ src/services/approvalRegistersApi.test.ts (5 tests) 15ms
 ✓ src/pages/borrower/portal/auth/MP00_Login.test.tsx (2 tests) 9ms
 ✓ src/services/navigationPermissions.test.ts (9 tests) 7ms
 ✓ src/services/loanPolicyApi.test.ts (2 tests) 9ms
 ✓ src/pages/profile/MyProfile.test.tsx (1 test) 27ms
 ✓ src/services/disbursementApi.test.ts (3 tests) 12ms
 ✓ src/contexts/RoleContext.test.tsx (2 tests) 6ms
 ✓ src/pages/applications/NomineeSelectionViews.test.tsx (3 tests) 7ms
 ✓ src/services/tracerApi.test.ts (2 tests) 4ms
 ✓ src/playwright.seed.test.ts (3 tests) 2ms
 ✓ src/utils/formatMoney.test.ts (1 test) 1ms
 ✓ src/utils/applicationDisplay.test.ts (5 tests) 2ms
 ✓ src/playwrightBrowser.test.ts (4 tests) 3ms
 ✓ src/services/demoAuthFlag.test.tsx (4 tests) 2590ms
   ✓ VITE_ENABLE_DEMO_AUTH real App/RoleProvider boundary (005FA4) > fails closed when the flag is unset  2375ms

 Test Files  58 passed (58)
      Tests  477 passed (477)
   Start at  01:14:51
   Duration  11.52s (transform 5.50s, setup 0ms, collect 16.50s, tests 38.48s, environment 9.71s, prepare 3.57s)


Duration milliseconds: 12066
Exit code: 0
