"""Review-only probes for 009L4 exact selector and pagination contracts.

The assertions express the selected slice's requirement that incoherent evidence
affects neither collection totals nor page reachability. They intentionally fail
while database candidate filters are broader than their owner-level projectors.
"""
...
class LoanAccountExactCountProbe(LoanAccountReadApiTests):
...
class SapCompletionExactCountProbe(ActiveLoanAccountReadApiTests):
...
class AssignedSapExactCountProbe(SapStaffWorkspaceApiTests):
...
class CfcExactCountProbe(DisbursementWorkspaceApiTests):
...
class PortalCanonicalSapEdgeProbe(SimpleTestCase):
    @patch(
