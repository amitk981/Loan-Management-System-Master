# test Results

Command: npm test --if-present

Node PATH pin: /Users/amitkallapa/.nvm/versions/node/v20.19.6/bin


> sfpcl-lms@1.0.0 test
> vitest run

[33mThe CJS build of Vite's Node API is deprecated. See https://vite.dev/guide/troubleshooting.html#vite-cjs-node-api-deprecated for more details.[39m

 RUN  v3.2.6 /Users/amitkallapa/LMS/.ralph/worktrees/2026-07-23_151537_normal_run/sfpcl-lms

 ✓ src/pages/registers/RegistersHub.test.tsx (8 tests) 901ms
   ✓ RegistersHub owned approval register panels > renders only the server-scoped frozen sanction page and replaces pagination after a filter change  338ms
 ✓ src/pages/settings/SettingsHub.test.tsx (10 tests) 1324ms
   ✓ SettingsHub Approval Matrix panel > permits a canonical manager to submit a complete successor version as a pending proposal  560ms
   ✓ SettingsHub remaining panels > creates a complete successor as a separate audited draft for a canonical policy manager  368ms
 ✓ src/pages/repayments/RepaymentsHub.test.tsx (9 tests) 1586ms
   ✓ 010MA Repayments Hub wiring > posts one governed direct attempt, displays backend allocation, and refreshes reads  521ms
   ✓ 010MA Repayments Hub wiring > shows backend posting denial without retry or canonical refresh: AuthSessionError: Amount must be a positive decimal.  336ms
   ✓ 010MA Repayments Hub wiring > shows backend posting denial without retry or canonical refresh: AuthSessionError: This bank reference is already recorded.  341ms
 ✓ src/pages/appraisal/AppraisalWorkbench.container.test.tsx (14 tests) 1996ms
   ✓ default AppraisalWorkbench authenticated HTTP container > calculates a limit from entered source IDs and refreshes four reads  378ms
   ✓ default AppraisalWorkbench authenticated HTTP container > posts the rejected Credit Manager decision once  310ms
 ✓ src/pages/documentation/DocumentationHub.test.tsx (18 tests) 2720ms
   ✓ 008M2 documentation workspace contract > posts a server-owned approval and refetches once without optimism  559ms
   ✓ 008M2 documentation workspace contract > keeps a conflict visible with no optimistic change, retry, or refetch  400ms
 ✓ src/pages/servicing/InterestMonitoringWorkspaces.test.tsx (6 tests) 667ms
   ✓ interest and monitoring workspaces > makes loan and invoice 101 reachable and accrues the disclosed complete selection  308ms
 ✓ src/pages/borrower/portal/applications/MP05_NewApplication.test.tsx (7 tests) 1169ms
   ✓ MP05 loan-limit display authority (006Z2 interim regression) > shows independent 400, 403, and 409 errors without retry or projection refetch  616ms
 ✓ src/pages/borrower/portal/documents/PortalDocumentationActions.test.tsx (8 tests) 615ms
   ✓ member portal documentation actions > renders server-owned actions and performs one canonical refetch after upload  370ms
 ✓ src/pages/loan-accounts/LoanAccount360.test.tsx (6 tests) 485ms
 ✓ src/pages/borrower/portal/PortalMemberViews.test.tsx (7 tests) 250ms
 ✓ src/components/layout/Header.notifications.test.tsx (9 tests) 469ms
 ✓ src/pages/borrower/portal/loans/PortalLoanViews.test.tsx (6 tests) 326ms
 ✓ src/pages/borrower/portal/disbursement/MP14_DisbursementStatus.test.tsx (6 tests) 421ms
 ✓ src/pages/borrower/portal/applications/MP11_DeficiencyResponse.test.tsx (2 tests) 605ms
   ✓ MP11 deficiency response > uploads a server-contracted response, refetches canonical state, and resubmits  525ms
 ✓ src/pages/search/GlobalSearchResults.test.tsx (5 tests) 617ms
 ✓ src/pages/sanction/SanctionWorkbench.test.tsx (39 tests) 6074ms
   ✓ SanctionWorkbench authenticated container > renders the authoritative total and replaces the queue from the next server page  517ms
   ✓ SanctionWorkbench authenticated container > keeps a newer filter authoritative while an action detail refresh finishes  374ms
   ✓ SanctionWorkbench authenticated container > keeps a newer empty filter authoritative while action submission finishes  310ms
   ✓ SanctionWorkbench authenticated container > keeps a newer denied state authoritative when an action detail refresh fails later  355ms
   ✓ SanctionWorkbench authenticated container > keeps a newer malformed state authoritative when an action detail refresh fails later  451ms
   ✓ SanctionWorkbench authenticated container > keeps a newer empty state authoritative when an action detail refresh fails later  304ms
   ✓ SanctionWorkbench authenticated container > keeps a newer filter authoritative while an action decision refresh finishes  375ms
   ✓ SanctionWorkbench authenticated container > requires a reason and sends the exact Reject request  374ms
   ✓ SanctionWorkbench authenticated container > requires a reason and sends the exact Return for Clarification request  424ms
   ✓ SanctionWorkbench authenticated container > uploads three application-scoped legal files before recording bounded special-case evidence  751ms
 ✓ src/pages/members/MemberGovernanceForm.test.tsx (5 tests) 2387ms
   ✓ MemberGovernanceForm > submits the institution create variant without individual identity/profile fields  1197ms
   ✓ MemberGovernanceForm > submits every individual registration profile field  892ms
 ✓ src/pages/applications/WitnessPanel.container.test.tsx (10 tests) 1242ms
   ✓ mounted witness resource actions > captures with the exact body and refetches the canonical collection once  432ms
 ✓ src/pages/disbursement/PaymentAuthorisationHub.test.tsx (5 tests) 1064ms
   ✓ 009K CFC payment authorisation workspace > shows CFC actions only when the backend projects them and posts the decision reason  377ms
   ✓ 009K CFC payment authorisation workspace > shows the truthful empty queue after an authorised item leaves CFC scope  316ms
   ✓ 009K CFC payment authorisation workspace > surfaces backend duplicate UTR and permission errors without optimistic success  345ms
 ✓ src/pages/disbursement/DisbursementHub.test.tsx (4 tests) 468ms
   ✓ 009K disbursement finance workspace > submits Money fields with one stable key and treats replay as success  313ms
 ✓ src/services/authSession.test.ts (39 tests) 39ms
 ✓ src/pages/members/MemberProfile.container.test.tsx (4 tests) 305ms
 ✓ src/pages/members/MemberGovernanceForm.container.test.tsx (16 tests) 5140ms
   ✓ MemberGovernanceForm production container > routes Directory registration into canonical Profile readback with the exact create ledger  2729ms
   ✓ MemberGovernanceForm production container > performs one ordinary human-like update before canonical Profile readback with the exact update ledger  376ms
   ✓ MemberGovernanceForm production container > posts only the protected identity delta through the shared HTTP transport  322ms
 ✓ src/services/servicingApi.test.ts (13 tests) 32ms
 ✓ src/services/portalApi.test.ts (10 tests) 24ms
 ✓ src/pages/members/MemberProfile.test.tsx (26 tests) 245ms
 ✓ src/services/applicationIntakeApi.test.ts (5 tests) 7ms
 ✓ src/pages/applications/ApplicationDetail.test.tsx (9 tests) 59ms
 ✓ src/services/creditAssessmentApi.test.ts (5 tests) 9ms
 ✓ src/pages/defaults/DefaultRecoveryHub.test.tsx (1 test) 707ms
   ✓ 011F recovery execution surface > uses the approved backend action and uploaded evidence to initiate S57  706ms
 ✓ src/components/layout/Header.search.test.tsx (1 test) 307ms
   ✓ 010N Header search path > navigates the transient query to S02 without building a local result index  306ms
 ✓ src/pages/members/Borrower360.test.tsx (4 tests) 173ms
 ✓ src/services/navigationPermissions.test.ts (9 tests) 25ms
 ✓ src/services/approvalRegistersApi.test.ts (5 tests) 11ms
 ✓ src/pages/appraisal/AppraisalWorkbench.test.tsx (22 tests) 296ms
 ✓ src/services/disbursementApi.test.ts (3 tests) 10ms
 ✓ src/services/tracerApi.test.ts (2 tests) 8ms
 ✓ src/pages/Dashboard.test.tsx (13 tests) 54ms
 ✓ src/services/loanPolicyApi.test.ts (2 tests) 7ms
 ✓ src/pages/applications/CompletenessWorkbench.test.tsx (5 tests) 68ms
 ✓ src/pages/notifications/NotificationsCenter.test.tsx (6 tests) 39ms
 ✓ src/pages/members/MemberDirectory.test.tsx (5 tests) 42ms
 ✓ src/utils/applicationDisplay.test.ts (5 tests) 5ms
 ✓ src/contexts/RoleContext.test.tsx (2 tests) 6ms
 ✓ src/pages/applications/ApplicationList.test.tsx (2 tests) 34ms
 ✓ src/pages/applications/NomineeSelectionViews.test.tsx (3 tests) 16ms
 ✓ src/playwrightBrowser.test.ts (4 tests) 6ms
 ✓ src/playwright.seed.test.ts (3 tests) 2ms
 ✓ src/utils/formatMoney.test.ts (1 test) 1ms
 ✓ src/pages/profile/MyProfile.test.tsx (1 test) 12ms
 ✓ src/pages/borrower/portal/auth/MP00_Login.test.tsx (2 tests) 8ms
 ✓ src/services/demoAuthFlag.test.tsx (3 tests) 1119ms
   ✓ VITE_ENABLE_DEMO_AUTH real App/RoleProvider boundary (005FA4) > fails closed when the flag is unset  1009ms

 Test Files  52 passed (52)
      Tests  415 passed (415)
   Start at  16:02:05
   Duration  10.77s (transform 4.83s, setup 0ms, collect 16.25s, tests 34.20s, environment 8.64s, prepare 3.57s)


Duration milliseconds: 11494
Exit code: 0
