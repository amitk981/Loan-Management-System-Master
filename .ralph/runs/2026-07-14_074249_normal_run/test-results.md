# test Results

Command: npm test --if-present

Node PATH pin: /Users/amitkallapa/.nvm/versions/node/v18.20.4/bin


> sfpcl-lms@1.0.0 test
> vitest run

[33mThe CJS build of Vite's Node API is deprecated. See https://vite.dev/guide/troubleshooting.html#vite-cjs-node-api-deprecated for more details.[39m

 RUN  v3.2.6 /Users/amitkallapa/LMS/.ralph/worktrees/2026-07-14_074249_normal_run/sfpcl-lms

 ✓ src/pages/borrower/portal/applications/MP05_NewApplication.test.tsx (7 tests) 1225ms
   ✓ MP05 loan-limit display authority (006Z2 interim regression) > shows independent 400, 403, and 409 errors without retry or projection refetch  810ms
 ✓ src/pages/applications/WitnessPanel.container.test.tsx (10 tests) 1313ms
   ✓ mounted witness resource actions > captures with the exact body and refetches the canonical collection once  527ms
 ✓ src/pages/settings/SettingsHub.test.tsx (10 tests) 1389ms
   ✓ SettingsHub Approval Matrix panel > permits a canonical manager to submit a complete successor version as a pending proposal  674ms
   ✓ SettingsHub remaining panels > creates a complete successor as a separate audited draft for a canonical policy manager  382ms
 ✓ src/pages/appraisal/AppraisalWorkbench.container.test.tsx (14 tests) 1876ms
   ✓ default AppraisalWorkbench authenticated HTTP container > calculates a limit from entered source IDs and refreshes four reads  382ms
 ✓ src/pages/members/MemberGovernanceForm.test.tsx (5 tests) 2462ms
   ✓ MemberGovernanceForm > submits the institution create variant without individual identity/profile fields  1228ms
   ✓ MemberGovernanceForm > submits every individual registration profile field  964ms
 ✓ src/pages/members/MemberProfile.container.test.tsx (4 tests) 270ms
 ✓ src/pages/members/MemberProfile.test.tsx (26 tests) 194ms
 ✓ src/pages/registers/RegistersHub.test.tsx (8 tests) 724ms
 ✓ src/pages/members/Borrower360.test.tsx (4 tests) 80ms
 ✓ src/pages/appraisal/AppraisalWorkbench.test.tsx (22 tests) 170ms
 ✓ src/pages/applications/ApplicationDetail.test.tsx (9 tests) 70ms
 ✓ src/pages/members/MemberDirectory.test.tsx (5 tests) 34ms
 ✓ src/pages/applications/CompletenessWorkbench.test.tsx (5 tests) 57ms
 ✓ src/pages/borrower/portal/PortalMemberViews.test.tsx (4 tests) 49ms
 ✓ src/pages/Dashboard.test.tsx (13 tests) 44ms
 ✓ src/pages/sanction/SanctionWorkbench.test.tsx (33 tests) 3632ms
   ✓ SanctionWorkbench authenticated container > loads frozen case truth and approves through the exact case boundary before canonical refresh  376ms
   ✓ SanctionWorkbench authenticated container > requires a reason and sends the exact Reject request  520ms
   ✓ SanctionWorkbench authenticated container > requires a reason and sends the exact Return for Clarification request  448ms
   ✓ SanctionWorkbench authenticated container > uploads three application-scoped legal files before recording bounded special-case evidence  630ms
 ✓ src/services/authSession.test.ts (36 tests) 29ms
 ✓ src/pages/notifications/NotificationsCenter.test.tsx (6 tests) 38ms
 ✓ src/services/portalApi.test.ts (4 tests) 10ms
 ✓ src/pages/applications/ApplicationList.test.tsx (2 tests) 29ms
 ✓ src/pages/profile/MyProfile.test.tsx (1 test) 12ms
 ✓ src/pages/borrower/portal/auth/MP00_Login.test.tsx (2 tests) 9ms
 ✓ src/contexts/RoleContext.test.tsx (2 tests) 11ms
 ✓ src/services/creditAssessmentApi.test.ts (5 tests) 12ms
 ✓ src/services/applicationIntakeApi.test.ts (5 tests) 14ms
 ✓ src/services/navigationPermissions.test.ts (9 tests) 18ms
 ✓ src/pages/applications/NomineeSelectionViews.test.tsx (3 tests) 9ms
 ✓ src/services/approvalRegistersApi.test.ts (5 tests) 7ms
 ✓ src/services/loanPolicyApi.test.ts (2 tests) 4ms
 ✓ src/services/tracerApi.test.ts (2 tests) 4ms
 ✓ src/utils/applicationDisplay.test.ts (5 tests) 2ms
 ✓ src/services/demoAuthFlag.test.tsx (3 tests) 2829ms
   ✓ VITE_ENABLE_DEMO_AUTH real App/RoleProvider boundary (005FA4) > fails closed when the flag is unset  2329ms
 ✓ src/pages/members/MemberGovernanceForm.container.test.tsx (16 tests) 3619ms
   ✓ MemberGovernanceForm production container > routes Directory registration into canonical Profile readback with the exact create ledger  2075ms

 Test Files  33 passed (33)
      Tests  287 passed (287)
   Start at  08:08:08
   Duration  6.68s (transform 5.00s, setup 0ms, collect 9.30s, tests 20.25s, environment 3.47s, prepare 2.04s)

