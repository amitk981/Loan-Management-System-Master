# test Results

Command: npm test --if-present

Node PATH pin: /Users/amitkallapa/.nvm/versions/node/v18.20.4/bin


> sfpcl-lms@1.0.0 test
> vitest run

[33mThe CJS build of Vite's Node API is deprecated. See https://vite.dev/guide/troubleshooting.html#vite-cjs-node-api-deprecated for more details.[39m

 RUN  v3.2.6 /Users/amitkallapa/LMS/.ralph/worktrees/2026-07-12_223530_normal_run/sfpcl-lms

 ✓ src/pages/members/MemberProfile.test.tsx (26 tests) 210ms
 ✓ src/pages/members/MemberProfile.container.test.tsx (4 tests) 257ms
 ✓ src/pages/appraisal/AppraisalWorkbench.test.tsx (22 tests) 157ms
 ✓ src/pages/members/Borrower360.test.tsx (4 tests) 91ms
 ✓ src/pages/applications/CompletenessWorkbench.test.tsx (5 tests) 64ms
 ✓ src/pages/applications/ApplicationDetail.test.tsx (9 tests) 63ms
 ✓ src/pages/applications/WitnessPanel.container.test.tsx (10 tests) 1112ms
   ✓ mounted witness resource actions > captures with the exact body and refetches the canonical collection once  393ms
 ✓ src/pages/appraisal/AppraisalWorkbench.container.test.tsx (14 tests) 1423ms
 ✓ src/pages/borrower/portal/PortalMemberViews.test.tsx (4 tests) 35ms
 ✓ src/pages/Dashboard.test.tsx (13 tests) 40ms
 ✓ src/pages/notifications/NotificationsCenter.test.tsx (6 tests) 29ms
 ✓ src/services/authSession.test.ts (15 tests) 14ms
 ✓ src/pages/members/MemberDirectory.test.tsx (5 tests) 30ms
 ✓ src/pages/applications/ApplicationList.test.tsx (2 tests) 31ms
 ✓ src/pages/profile/MyProfile.test.tsx (1 test) 11ms
 ✓ src/services/demoAuthFlag.test.tsx (3 tests) 2579ms
   ✓ VITE_ENABLE_DEMO_AUTH real App/RoleProvider boundary (005FA4) > fails closed when the flag is unset  2218ms
 ✓ src/pages/members/MemberGovernanceForm.test.tsx (4 tests) 1994ms
   ✓ MemberGovernanceForm > submits the institution create variant without individual identity/profile fields  1032ms
   ✓ MemberGovernanceForm > submits every individual registration profile field  803ms
 ✓ src/contexts/RoleContext.test.tsx (2 tests) 10ms
 ✓ src/services/tracerApi.test.ts (2 tests) 4ms
 ✓ src/services/applicationIntakeApi.test.ts (5 tests) 8ms
 ✓ src/services/navigationPermissions.test.ts (7 tests) 7ms
 ✓ src/services/creditAssessmentApi.test.ts (5 tests) 7ms
 ✓ src/pages/borrower/portal/auth/MP00_Login.test.tsx (2 tests) 11ms
 ✓ src/services/portalApi.test.ts (4 tests) 6ms
 ✓ src/pages/sanction/SanctionWorkbench.test.tsx (2 tests) 7ms
 ✓ src/pages/applications/NomineeSelectionViews.test.tsx (3 tests) 8ms
 ✓ src/utils/applicationDisplay.test.ts (5 tests) 5ms
 ✓ src/pages/borrower/portal/applications/MP05_NewApplication.test.tsx (2 tests) 1ms
 ✓ src/pages/members/MemberGovernanceForm.container.test.tsx (13 tests) 4772ms
   ✓ MemberGovernanceForm production container > submits the complete individual_farmer body through the shared HTTP transport  1958ms
   ✓ MemberGovernanceForm production container > submits the complete fpc body through the shared HTTP transport  1110ms
   ✓ MemberGovernanceForm production container > submits the complete producer_institution body through the shared HTTP transport  1100ms

 Test Files  29 passed (29)
      Tests  199 passed (199)
   Start at  22:44:14
   Duration  5.77s (transform 2.58s, setup 0ms, collect 4.87s, tests 12.99s, environment 1.89s, prepare 1.70s)

