# test Results

Command: npm test --if-present

Node PATH pin: /Users/amitkallapa/.nvm/versions/node/v20.19.6/bin


> sfpcl-lms@1.0.0 test
> vitest run

[33mThe CJS build of Vite's Node API is deprecated. See https://vite.dev/guide/troubleshooting.html#vite-cjs-node-api-deprecated for more details.[39m

 RUN  v3.2.6 /Users/amitkallapa/LMS/.ralph/worktrees/2026-07-24_041849_normal_run/sfpcl-lms

 ✓ src/pages/registers/RegistersHub.test.tsx (8 tests) 837ms
 ✓ src/pages/settings/SettingsHub.test.tsx (10 tests) 1249ms
   ✓ SettingsHub Approval Matrix panel > permits a canonical manager to submit a complete successor version as a pending proposal  581ms
   ✓ SettingsHub remaining panels > creates a complete successor as a separate audited draft for a canonical policy manager  344ms
 ✓ src/pages/repayments/RepaymentsHub.test.tsx (9 tests) 1561ms
   ✓ 010MA Repayments Hub wiring > posts one governed direct attempt, displays backend allocation, and refreshes reads  494ms
   ✓ 010MA Repayments Hub wiring > shows backend posting denial without retry or canonical refresh: AuthSessionError: Amount must be a positive decimal.  303ms
   ✓ 010MA Repayments Hub wiring > shows backend posting denial without retry or canonical refresh: AuthSessionError: This bank reference is already recorded.  336ms
 ✓ src/pages/appraisal/AppraisalWorkbench.container.test.tsx (14 tests) 1851ms
   ✓ default AppraisalWorkbench authenticated HTTP container > calculates a limit from entered source IDs and refreshes four reads  367ms
   ✓ default AppraisalWorkbench authenticated HTTP container > posts the rejected Credit Manager decision once  304ms
 ✓ src/pages/servicing/InterestMonitoringWorkspaces.test.tsx (6 tests) 423ms
 ✓ src/pages/borrower/portal/applications/MP05_NewApplication.test.tsx (7 tests) 1041ms
   ✓ MP05 loan-limit display authority (006Z2 interim regression) > shows independent 400, 403, and 409 errors without retry or projection refetch  526ms
 ✓ src/pages/documentation/DocumentationHub.test.tsx (18 tests) 2819ms
   ✓ 008M2 documentation workspace contract > posts a server-owned approval and refetches once without optimism  506ms
   ✓ 008M2 documentation workspace contract > keeps a conflict visible with no optimistic change, retry, or refetch  494ms
   ✓ 008M2 documentation workspace contract > submits a signed-copy upload through the opaque action and refetches once  398ms
 ✓ src/pages/borrower/portal/documents/PortalDocumentationActions.test.tsx (8 tests) 658ms
   ✓ member portal documentation actions > renders server-owned actions and performs one canonical refetch after upload  419ms
 ✓ src/pages/loan-accounts/LoanAccount360.test.tsx (6 tests) 444ms
 ✓ src/pages/borrower/portal/PortalMemberViews.test.tsx (7 tests) 257ms
 ✓ src/pages/borrower/portal/loans/PortalLoanViews.test.tsx (6 tests) 329ms
 ✓ src/components/layout/Header.notifications.test.tsx (9 tests) 521ms
 ✓ src/pages/borrower/portal/disbursement/MP14_DisbursementStatus.test.tsx (6 tests) 302ms
 ✓ src/pages/borrower/portal/applications/MP11_DeficiencyResponse.test.tsx (2 tests) 590ms
   ✓ MP11 deficiency response > uploads a server-contracted response, refetches canonical state, and resubmits  503ms
 ✓ src/pages/search/GlobalSearchResults.test.tsx (5 tests) 516ms
 ✓ src/pages/sanction/SanctionWorkbench.test.tsx (39 tests) 5666ms
   ✓ SanctionWorkbench authenticated container > renders the authoritative total and replaces the queue from the next server page  552ms
   ✓ SanctionWorkbench authenticated container > keeps a newer filter authoritative while an action detail refresh finishes  427ms
   ✓ SanctionWorkbench authenticated container > keeps a newer empty filter authoritative while action submission finishes  314ms
   ✓ SanctionWorkbench authenticated container > keeps a newer denied state authoritative when an action detail refresh fails later  334ms
   ✓ SanctionWorkbench authenticated container > keeps a newer malformed state authoritative when an action detail refresh fails later  345ms
   ✓ SanctionWorkbench authenticated container > keeps a newer empty state authoritative when an action detail refresh fails later  309ms
   ✓ SanctionWorkbench authenticated container > keeps a newer filter authoritative while an action decision refresh finishes  355ms
   ✓ SanctionWorkbench authenticated container > requires a reason and sends the exact Reject request  379ms
   ✓ SanctionWorkbench authenticated container > requires a reason and sends the exact Return for Clarification request  324ms
   ✓ SanctionWorkbench authenticated container > uploads three application-scoped legal files before recording bounded special-case evidence  631ms
 ✓ src/pages/borrower/portal/PortalCommunicationsViews.test.tsx (5 tests) 686ms
   ✓ MP19-MP24 member communication views > renders all MP24 guide sections, validates required fields, submits, and shows resolution  468ms
 ✓ src/pages/applications/WitnessPanel.container.test.tsx (10 tests) 1128ms
   ✓ mounted witness resource actions > captures with the exact body and refetches the canonical collection once  363ms
 ✓ src/pages/members/MemberGovernanceForm.test.tsx (5 tests) 2198ms
   ✓ MemberGovernanceForm > submits the institution create variant without individual identity/profile fields  1157ms
   ✓ MemberGovernanceForm > submits every individual registration profile field  814ms
 ✓ src/pages/members/MemberGovernanceForm.container.test.tsx (16 tests) 4437ms
   ✓ MemberGovernanceForm production container > routes Directory registration into canonical Profile readback with the exact create ledger  2379ms
 ✓ src/pages/disbursement/DisbursementHub.test.tsx (4 tests) 411ms
 ✓ src/pages/disbursement/PaymentAuthorisationHub.test.tsx (5 tests) 958ms
   ✓ 009K CFC payment authorisation workspace > shows CFC actions only when the backend projects them and posts the decision reason  314ms
   ✓ 009K CFC payment authorisation workspace > surfaces backend duplicate UTR and permission errors without optimistic success  402ms
 ✓ src/pages/compliance/AuditorEpic011View.test.tsx (4 tests) 226ms
 ✓ src/services/authSession.test.ts (39 tests) 34ms
 ✓ src/pages/members/MemberProfile.test.tsx (26 tests) 168ms
 ✓ src/pages/members/MemberProfile.container.test.tsx (4 tests) 327ms
 ✓ src/services/servicingApi.test.ts (13 tests) 37ms
 ✓ src/services/portalApi.test.ts (10 tests) 29ms
 ✓ src/components/layout/Header.search.test.tsx (1 test) 239ms
 ✓ src/services/applicationIntakeApi.test.ts (5 tests) 8ms
 ✓ src/services/creditAssessmentApi.test.ts (5 tests) 7ms
 ✓ src/pages/applications/ApplicationDetail.test.tsx (9 tests) 70ms
 ✓ src/services/approvalRegistersApi.test.ts (5 tests) 8ms
 ✓ src/pages/defaults/DefaultRecoveryHub.test.tsx (1 test) 616ms
   ✓ 011F recovery execution surface > uses the approved backend action and uploaded evidence to initiate S57  615ms
 ✓ src/services/navigationPermissions.test.ts (9 tests) 11ms
 ✓ src/pages/members/Borrower360.test.tsx (4 tests) 105ms
 ✓ src/pages/appraisal/AppraisalWorkbench.test.tsx (22 tests) 204ms
 ✓ src/pages/applications/CompletenessWorkbench.test.tsx (5 tests) 72ms
 ✓ src/services/disbursementApi.test.ts (3 tests) 6ms
 ✓ src/services/tracerApi.test.ts (2 tests) 6ms
 ✓ src/pages/Dashboard.test.tsx (13 tests) 51ms
 ✓ src/pages/notifications/NotificationsCenter.test.tsx (6 tests) 48ms
 ✓ src/services/loanPolicyApi.test.ts (2 tests) 7ms
 ✓ src/pages/members/MemberDirectory.test.tsx (5 tests) 60ms
 ✓ src/utils/applicationDisplay.test.ts (5 tests) 3ms
 ✓ src/contexts/RoleContext.test.tsx (2 tests) 10ms
 ✓ src/pages/applications/ApplicationList.test.tsx (2 tests) 42ms
 ✓ src/pages/applications/NomineeSelectionViews.test.tsx (3 tests) 7ms
 ✓ src/pages/profile/MyProfile.test.tsx (1 test) 14ms
 ✓ src/playwright.seed.test.ts (3 tests) 2ms
 ✓ src/utils/formatMoney.test.ts (1 test) 2ms
 ✓ src/pages/borrower/portal/auth/MP00_Login.test.tsx (2 tests) 10ms
 ✓ src/playwrightBrowser.test.ts (4 tests) 2ms
 ✓ src/services/demoAuthFlag.test.tsx (3 tests) 1154ms
   ✓ VITE_ENABLE_DEMO_AUTH real App/RoleProvider boundary (005FA4) > fails closed when the flag is unset  1044ms

 Test Files  54 passed (54)
      Tests  424 passed (424)
   Start at  05:05:45
   Duration  10.33s (transform 4.40s, setup 0ms, collect 15.59s, tests 32.47s, environment 8.29s, prepare 3.42s)


Duration milliseconds: 11097
Exit code: 0
